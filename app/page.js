    ...
    <div className="flex flex-col bg-white">
              ...
              <div className="relative h-full w-full group">
                ...
                <div className="absolute inset-0 bg-black/10 flex flex-col items-center justify-center text-center px-4">
                  <div className="animate-fadeIn space-y-4">
                    <h2 className="text-white text-sm tracking-[0.4em] font-light mb-2">{slide.subtitle}</h2>
                    <h1 className="text-white text-4xl md:text-7xl font-light tracking-[0.2em] mb-8">
                      {slide.title}
                    </h1>
                    <Link 
                      href={slide.link} 
                      className="inline-block border border-white text-white px-10 py-3 text-xs tracking-[0.3em] font-medium hover:bg-white hover:text-black transition-all duration-300 uppercase"
                    >
                      Shop Now
                    </Link>
                  </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 md:p-8">
        {categoryBanners.map((cat, idx) => (
          <Link key={idx} href={cat.link} className="relative aspect-[4/5] md:aspect-[16/9] overflow-hidden group">
            <Image 
              src={cat.image} 
              alt={cat.name} 
              fill 
            ...
            <div className="absolute inset-0 bg-black/5 group-hover:bg-black/20 transition-all flex items-center justify-center">
              <div className="text-center">
                <Link 
                  href={cat.link}
                  className="bg-white/90 text-black px-8 py-3 text-xs tracking-[0.3em] font-semibold hover:bg-black hover:text-white transition-all duration-300"
                >
                  SHOP NOW {cat.name}
                </Link>
              </div>
      ...
      {/* Three Ways to Wear Section */}
      <section className="py-20 px-4 md:px-8 text-center bg-gray-50">
        <h2 className="text-2xl md:text-4xl font-light tracking-[0.2em] text-gray-800 mb-4">THREE Ways to Wear Elegance</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-7xl mx-auto">
          {[
            ...
            <div key={idx} className="space-y-6 group">
              ...
              <h3 className="text-lg font-light tracking-[0.2em]">{item.title}</h3>
              <Link href="/products" className="inline-block text-[10px] tracking-[0.3em] font-bold border-b border-black pb-1 hover:text-gray-500 hover:border-gray-500 transition-colors">
                DISCOVER MORE
              </Link>
            </div>
          ))}
      ...
      {/* Most Popular Picks (Carousel) */}
      <section className="py-24 px-4 md:px-8 max-w-[1600px] mx-auto w-full">
        <div className="text-center mb-16">
          <h2 className="text-2xl md:text-4xl font-light tracking-[0.2em] text-gray-800 uppercase">Discover our most-popular picks</h2>
        </div>
              ...
              <SwiperSlide key={product._id}>
                <div className="group cursor-pointer">
                  <Link href={`/products/${product._id}`}>
                    <div className="relative aspect-[3/4] overflow-hidden bg-gray-50 mb-6">
                      ...
                      {product.salePrice && product.salePrice < product.price && (
                        <div className="absolute top-4 left-4 bg-black text-white text-[10px] px-3 py-1 tracking-widest uppercase">
                          Sale
                        </div>
                      )}
                      <div className="absolute inset-x-0 bottom-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                        <button className="w-full bg-white/90 text-black py-3 text-[10px] tracking-[0.2em] font-bold hover:bg-black hover:text-white transition-colors">
                          QUICK ADD
                        </button>
                      </div>
                    </div>
                  ...
                  <div className="text-center space-y-2">
                    <h3 className="text-[11px] tracking-[0.1em] text-gray-500 uppercase">{product.category?.name || 'Collection'}</h3>
                    <Link href={`/products/${product._id}`}>
                      <h4 className="text-sm font-medium text-gray-800 hover:text-gray-500 transition-colors">{product.name}</h4>
                    </Link>
                    <div className="flex items-center justify-center space-x-3 text-sm tracking-widest">
                      {product.salePrice && product.salePrice < product.price ? (
                        <>
                          <span className="text-red-600">Rs. {product.salePrice.toLocaleString()}</span>
                          <span className="text-gray-400 line-through">Rs. {product.price.toLocaleString()}</span>
                        </>
                      ) : (
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-1 p-1 bg-white">
        {[
          ...
          <div key={idx} className="relative aspect-[3/4] overflow-hidden group">
            ...
            />
            <div className="absolute inset-0 bg-black/10 group-hover:bg-black/30 transition-all flex flex-col items-center justify-center p-8">
              <h3 className="text-white text-xl md:text-2xl font-light tracking-[0.2em] text-center mb-6">{item.label}</h3>
              <Link href="/products" className="bg-white text-black px-8 py-3 text-[10px] tracking-[0.2em] font-bold hover:bg-black hover:text-white transition-all duration-300">
                SHOP NOW
              </Link>
            </div>
          </div>
      ...
      {/* Newsletter Section */}
      <section className="py-32 px-4 md:px-8 bg-gray-50 border-y border-gray-100">
        <div className="max-w-4xl mx-auto text-center space-y-8">
          <p className="text-[10px] tracking-[0.4em] text-gray-500 uppercase font-semibold">Keep me updated</p>
          <h2 className="text-3xl md:text-5xl font-light tracking-[0.1em] text-gray-900">Newsletter</h2>
          ...
          <form className="flex flex-col md:flex-row items-center justify-center gap-4 max-w-2xl mx-auto mt-10">
            <input 
              type="email" 
              placeholder="Email address" 
              className="w-full bg-white border border-gray-200 px-6 py-4 text-sm focus:outline-none focus:border-black transition-colors font-light"
              required
            />
            <button 
              type="submit" 
              className="w-full md:w-auto bg-black text-white px-12 py-4 text-[10px] tracking-[0.3em] font-bold hover:bg-gray-800 transition-colors uppercase whitespace-nowrap"
            >
              Subscribe
            </button>
          </form>
